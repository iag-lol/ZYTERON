...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!city) {
    return createPageMetadata({
      ...
      path: `/paginas-web-para-pymes/${ubicacion}`,
      noIndex: true,
    });
  }

  const page = buildLocalServicePageModel(localServiceDefinitions["paginas-web-para-pymes"], city);
  return createPageMetadata({
...
export default async function PaginasWebPymeUbicacionPage({ params }: Props) {
  const { ubicacion } = await params;
  const city = getUbicacionBySlug(ubicacion);
  if (!city) notFound();
  const page = buildLocalServicePageModel(localServiceDefinitions["paginas-web-para-pymes"], city);
