            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {profiles.map((profile) => {
                ...
                return (
                  <article
                    key={profile.id}
                    className="group relative flex flex-col overflow-hidden rounded-[1.9rem] border border-slate-200 bg-white shadow-[0_30px_60px_-42px_rgba(15,23,42,0.4)] transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-[0_32px_65px_-40px_rgba(37,99,235,0.32)]"
                  >
                    <div
                    ...
                    <div className="flex flex-1 flex-col p-6">
                      <div className="mb-5 flex flex-wrap items-center gap-2">
                        <span className="rounded-full border border-blue-100 bg-blue-50 px-4 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-blue-700">
                          Rubro
                        </span>
                        {profile.industry ? (
                      ...
                      <div className="relative mb-5 flex h-52 items-center justify-center overflow-hidden rounded-[1.7rem] border border-slate-200 bg-[radial-gradient(circle_at_top,#dbeafe,white_70%)] shadow-inner sm:h-60">
                        {profile.publicLogoUrl ? (
                          <Image
                            src={profile.publicLogoUrl}
                            alt={`Logo de ${profile.businessName}`}
                            fill
                            sizes="(max-width: 768px) 90vw, (max-width: 1280px) 40vw, 360px"
                            className="object-contain p-4 sm:p-5"
                          />
                        ) : (
                          <div
                            className={`flex h-full w-full items-center justify-center bg-gradient-to-br ${gradient} text-4xl font-extrabold text-white`}
                          >
                            {initials}
                          </div>
                        )}
                      ...
                      {profile.publicDescription && (
                        <div className="mb-5 rounded-[1.45rem] border border-slate-200 bg-slate-50/80 p-4">
                      ...
                      <div className="mt-auto grid gap-3 sm:grid-cols-2">
                        {(profile.comuna || profile.region) && (
                          <div className="rounded-[1.35rem] border border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f8fafc)] p-4">
                        ...
                        {igHandle ? (
                          ...
                          >
                            ...
                            <p className="mt-2 text-sm font-semibold text-pink-600 transition-colors hover:text-pink-500">
                              @{igHandle}
                            </p>
                          </a>
                        ) : null}
                      ...
                      {profile.publishedAt && (
                        <p className="pt-5 text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">
