...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!city) {
    return createPageMetadata({
      ...
      path: `/soporte-ti/${ubicacion}`,
      noIndex: true,
    });
  }

  const page = buildLocalServicePageModel(localServiceDefinitions["soporte-ti"], city);
  return createPageMetadata({
...
export default async function SoporteTiUbicacionPage({ params }: Props) {
  const { ubicacion } = await params;
  const city = getUbicacionBySlug(ubicacion);
  if (!city) notFound();
  const page = buildLocalServicePageModel(localServiceDefinitions["soporte-ti"], city);
