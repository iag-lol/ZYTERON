...
const navGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        href: "/admin/ventas-ia/configuracion",
        label: "Configuración",
  ...
  },
  {
    label: "Operaciones",
    items: [
      { href: "/admin/operaciones", label: "Centro de operaciones", icon: Activity, color: "blue" },
      {
  ...
  },
  {
    ...
    items: [
      {
        href: "/admin/contador-auditor",
        label: "Contador Auditor",
        icon: Calculator,
        color: "blue",
      },
      {
        href: "/admin/contador-auditor/facturacion",
  ...
  {
    label: "Programas",
    items: [{ href: "/admin/becas", label: "Becas Web Pyme", icon: Award, color: "blue" }],
  },
  {
    label: "Sistema",
    items: [
...
const colorMap: Record<
  string,
  { activeBg: string; activeText: string; activeIcon: string; activeDot: string }
> = {
  blue: {
    activeBg: "bg-blue-50",
...
export function AdminSidebar() {
  ...
  const activeGroupLabel = navGroups.find((g) =>
    g.items.some((i) => isActive(i.href, i.exact)),
  )?.label;
      ...
      >
        ...
        <div className="relative flex h-16 shrink-0 items-center border-b border-slate-200 px-4">
          ...
          <button
            ...
            className="absolute top-1/2 right-2 hidden h-6 w-6 -translate-y-1/2 items-center justify-center rounded-md border border-slate-200 bg-slate-50 text-slate-400 transition-all hover:border-blue-300 hover:bg-blue-50 hover:text-blue-600 lg:flex"
          ...
          <button
            onClick={() => setMobileNavOpen(false)}
            className="absolute top-1/2 right-2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 lg:hidden"
        ...
        {/* Navigation */}
        <nav className="scrollbar-none flex-1 overflow-x-hidden overflow-y-auto px-2 py-3">
          <div className="space-y-1.5">
            {navGroups.map((group) => {
              const isGroupOpen =
                !showLabels || group.label === activeGroupLabel || !collapsedGroups[group.label];
              return (
                <div key={group.label}>
                  {showLabels ? (
                    <button
                      ...
                      className="mb-0.5 flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-[10px] font-bold tracking-[0.14em] text-slate-400 uppercase transition-colors hover:bg-slate-50 hover:text-slate-600"
                    >
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {active && (
                            <span
                              className={`absolute inset-y-1.5 left-0 w-[3px] rounded-r-full ${c.activeDot}`}
                            />
                          )}
                          {/* Icon */}
                          <item.icon
        ...
        <div className="shrink-0 border-t border-slate-200 p-3">
          <div
            className={`flex items-center gap-3 rounded-lg px-2 py-2 ${!showLabels ? "justify-center" : ""}`}
          >
